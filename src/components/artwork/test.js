...
import { BlueMoonProContract,FudgeTokenContract,FudgeContract
...
const ArtWork = () => {
  ...
  const single = useSelector(
    (state) => state.CollectionReducer.GetSingletTokenData
  );
  ...
  const GetSingleData = async () => {
    ...
    try {
      await axios
        .post(`${API_URL}/token/getTokenAndDetailsOfSingleToken`, {
  ...
  const getMoreCreatorsNfts = async (creatorAddress) => {
    axios
      .post(`${API_URL}/token/getAllTokensOfCreator`, {
  ...
  const web3 = useWeb3();
  let tokenAddress = environment.BlueMoonPro;
  const Fudge = environment.FudgeToken;
  const Fcontract = FudgeTokenContract(Fudge, web3)
  const fudgeMarketplace = environment.Fudge;

//   const contractForProApproval=BlueMoonProContract(tokenAddress,web3)
    ...
    if (account) {
      ...
      try {      
        ...
        if (fudgeDropDown === "FUDGE") {
          ...
          setOpen(true);
          const fudgeSale = await FudgeSale(
            tokenId,
            singleData[0]?.orders[0]?.price
          );
          if (fudgeSale.status) {
            setOpen(false);
            await AddOrder(
    ...
    if (account) {
      try {
        ...
        if (singleData[0]?.orders[0]?.currency === "FUDGE") {
          ...
          const FudgePrice = await approveForFudge(
            singleData[0]?.orders[0]?.price
          );
  ...
  const MoreCreatorNfts = more?.slice(0, count)?.map((elem, index) => {
    const creator = elem?.creators.map((elem) => {
      return (
        <Link to={`/creatorprofile/${elem.walletAddress}`}>
          <img
            src={elem?.ipfsImageUrl}
            alt=""
            width="20px"
            height="20px"
            className="inner-tiless"
          />
        </Link>
      );
    });
    ...
    const price = elem.orders.map((elem) => {
      return <h6 className="clr">{elem?.price} BNB</h6>;
    });
        ...
        <div className="inner-card image-width">
          ...
          <Link to={`/artwork/${elem.contractAddress}/${elem.tokenID}`}>
            <img
              src={elem?.imageUrl}
              alt=""
              className="img-fluid mb10 set_width_height"
            />
            <h4>{elem?.nftName}</h4>
                        ...
                        <li className="">
                          <div className="icon-1">
                            <button data-dismiss="modal">
                              <img
                                src={single?.user?.ipfsImageUrl}
                                className="img-fluid img-1"
                                alt=""
                              />
                              &nbsp;&nbsp;
                              <span className="grey-1">
              ...
              <div className="art-inner">
                    ...
                    <div className="profile">
                      <div className="profile-heading">
                        <h2 className="inner-heading">
                          {singleData[0]?.nftName}
                        </h2>
                        <h4 className="inner-para">
                          {" "}
                          <span>
                            {singleData[0]?.orders[0]?.price
                              ? singleData[0]?.orders[0]?.price
                              : 0}{" "}
                            {singleData[0]?.orders[0]?.currency}
                          </span>
                        </h4>
                      </div>
                      <div className="icons">
                        <ul className="list-inline">
                          <li className="list-inline-item">
                            <img
                              src="/pegify/landing-assets/heart.png"
                              alt=""
                              className="img-fluid"
                            />
                            <img
                              src="pegify/heart-outline-icon.png"
                              alt=""
                              className="img-fluid"
                            />
                            <span className="grey">
                              {singleData[0]?.numerOfLikes}
                            </span>
                          </li>
                        </ul>
                      </div>
                ...
                <div className="row ptb20">
                  <div className="col-sm-12">
                    <div className="owner">
                      <Link
                        to={`/ownerprofile/${singleData[0]?.walletAddress}`}
                      >
                        <h6>Owner</h6>
                        <ul className="list-inline">
                          <li className="list-inline-item">
                            <img
                              src={singleData[0]?.users?.ipfsImageUrl}
                              alt=""
                ...
                <div className="row">
                  <div className="col-sm-12">
                    <div className="owner">
                      <Link
                        to={`/creatorprofile/${singleData[0]?.creators?.walletAddress}`}
                      >
                        <h6>Creator</h6>
                        <ul className="list-inline">
                          <li className="list-inline-item">
                            <img
                              src={singleData[0]?.creators?.ipfsImageUrl}
                              alt=""
                              className="img-fluid inner-imagess"
                            />
                          </li>
                          ...
                          <li className="list-inline-item grey-1">
                            {singleData[0]?.creators?.displayName}
                          </li>
                        </ul>
                      </Link>
                    </div>
                  ...
                  <div className="col-sm-12">
                    <div className="inner-btn">
                      ...
                      singleData[0]?.users?.walletAddress === account ? (
                        ...
                        singleData[0].orders[0]?.price <= 0 ? (
                        <button
                          type="button"
                          className="btn-common-1"
                          onClick={OpenPutOnsaleModal}
                        >
                          Put On Market Place
                        </button>
                      ) : singleData[0]?.orders[0] &&
                        ...
                        <button
                          ...
                          onClick={buyModal}
                        >
                          BUY NOW FOR {singleData[0]?.orders[0]?.price}{" "}
                          {singleData[0]?.orders[0]?.currency}
                        </button>
                      ) : singleData[0]?.orders[0] &&
                        singleData[0]?.users?.walletAddress === account &&
                ...
                <div className="modal-body">
                  <div className="row ptb20">
                    <div className="col-sm-4  main-upper-modal-image text-center">
                      <img
                        src={singleData[0]?.imageUrl}
                        className="img-fluid wdimagessks"
                        alt=""
                      />
                    </div>
                  ...
                  <div className="row main-margin-sho">
                    ...
                    <div className="col-sm-2 main-margin-sho"></div>
                  </div>
                  <hr />
                  <div className="row main-margin-sho">
                    ...
                    <div className="col-sm-3 main-margin-sho">
                      <h6>
                        <span>{singleData[0]?.orders[0]?.price} BNB</span>
                      </h6>
                    </div>
                  </div>
                  ...
                  <div className="row main-margin-sho">
                    ...
                    <div className="col-sm-3 ">
                      <h6>
                        <span className="clr">
                          {singleData[0]?.orders[0]?.price} BNB
                        </span>
                      </h6>
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-sm-12 text-center">
                      <ul className="list-inline">
                        <li className="list-inline-item">
                          <button
                            className={terms ? "btn-common" : "btn-common1"}
                            onClick={BuyNft}
                          >
                            Proceed to Payment
                          </button>
                          ...
                          <div className="modal-dialog" role="document">
                            <div className="modal-content">
                              ...
                              <div className="modal-body">
                                <div className="row ptb20">
                                  <div className="col-sm-12 text-center">
                                    <h4>Your NFT Put On Market Successfully</h4>
                                  </div>
                                </div>
                              </div>
          ...
          <div className="row">
            {MoreCreatorNfts.length > 0 && (
              <div className="col-sm-12">
                <h2>
                  {" "}
                  <span className="clr">MORE FROM</span> THIS CREATOR
                </h2>
                <div className="row ptb20">{MoreCreatorNfts}</div>
                <div className="row ptb20">
                  <div className="col-sm-12 text-center">
                    <ul className="list-inline">
                      <li className="list-inline-item">
                        {more.length > count ? (
                          <button className="btn-common" onClick={LoadMore}>
                            LOAD MORE
                          </button>
                        ) : (
